...
class DataDeclTestBase : public testing::Test {
 protected:
  ...
  static types::Type MakeTestEntityType(types::TypeFactory& factory) {
    const types::Schema& schema = factory.RegisterSchema("TestStruct", {});
...
TEST_F(DataDeclTest, PrimitiveTypeDecl) {
  DataDecl primitive_decl("count", type_factory_.MakePrimitiveType());
  EXPECT_EQ(primitive_decl.name(), "count");
}

TEST_F(DataDeclTest, EntityTypeDecl) {
  DataDecl entity_decl("person",
                       DataDeclTest::MakeTestEntityType(type_factory_));
...
TEST_F(DataDeclCollectionTest, AddDeclReturnsAddedDecl) {
  ...
  EXPECT_EQ(decl.type().type_base().kind(), types::TypeBase::Kind::kPrimitive);
}
...
//   EXPECT_EQ(decl.name(), "count");
//   EXPECT_EQ(decl.type().type_base().kind(),
